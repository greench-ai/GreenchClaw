...
function fontFaceCSS() {
  ...
  if (virgil) {
    fonts.push(
      `@font-face { font-family: "Virgil"; src: url("data:font/woff2;base64,${virgil}") format("woff2"); }`,
    );
  }
  const cascadia = loadFontBase64("CascadiaCode.woff2");
  if (cascadia) {
    fonts.push(
      `@font-face { font-family: "Cascadia"; src: url("data:font/woff2;base64,${cascadia}") format("woff2"); }`,
    );
  }
  return fonts.join("\n");
}
...
function getFontFamily(ff) {
  switch (ff) {
    ...
    case 3:
      return '"Cascadia", "Fira Code", monospace';
    default:
      return '"Virgil", sans-serif';
  }
}
  ...
  for (const el of elements) {
    ...
    if (el.points && el.points.length > 2) {
      ...
      if (startEl) {
        ...
        absPoints[0] = [
          sEdge2.x + ARROW_GAP * Math.cos(angle2),
          sEdge2.y + ARROW_GAP * Math.sin(angle2),
        ];
      }
      if (endEl) {
        ...
        absPoints[absPoints.length - 1] = [
          eEdge2.x - ARROW_GAP * Math.cos(angle2),
          eEdge2.y - ARROW_GAP * Math.sin(angle2),
        ];
      }
...
function main() {
  ...
  if (!outputPath) {
    if (inputPath === "-") {
      ...
      outputPath = resolve(
        dirname(inputPath),
        basename(inputPath).replace(/\.[^.]+$/, "") + ".png",
      );
    }
  }
  ...
  const fontFiles = [];
  try {
    fontFiles.push(join(FONT_DIR, "Virgil.ttf"));
  } catch {}
  try {
    fontFiles.push(join(FONT_DIR, "CascadiaCode.ttf"));
  } catch {}
  ...
  try {
    ...
    console.log(
      `✓ Rendered ${elements.length} elements → ${outputPath} (${Math.ceil(width * SCALE)}×${Math.ceil(height * SCALE)}px)`,
    );
  } catch (err) {
