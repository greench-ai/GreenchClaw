  ...
  it("logs process poll timeout context separately from session idle", async () => {
    ...
    const response = __testing.handleDynamicToolCallWithTimeout({
      call: {
        ...
        namespace: null,
        tool: "process",
        arguments: { action: "poll", sessionId: "rapid-cannabis-leaf", timeout: 30_000 },
      },
      toolBridge: {
    ...
    await expect(response).resolves.toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: "GreenchClaw dynamic tool call timed out after 1ms while waiting for process action=poll sessionId=rapid-cannabis-leaf. This is a tool RPC timeout, not a session idle timeout.",
        },
      ],
    });
    expect(warn).toHaveBeenCalledWith("codex dynamic tool call timed out", {
      ...
      processAction: "poll",
      processSessionId: "rapid-cannabis-leaf",
      processRequestedTimeoutMs: 30_000,
      consoleMessage:
        "codex process tool timeout: action=poll sessionId=rapid-cannabis-leaf toolTimeoutMs=1 requestedWaitMs=30000; per-tool-call watchdog, not session idle; repeated lines usually mean process-poll retry churn, not model progress",
    });
  });
