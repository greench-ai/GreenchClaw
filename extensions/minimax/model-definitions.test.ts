...
describe("minimax model definitions", () => {
  ...
  it("M3 advertises the 1M-token context window upstream", () => {
    expect(MINIMAX_M3_CONTEXT_WINDOW).toBe(1_000_000);
    expect(MINIMAX_M3_MAX_TOKENS).toBe(64_000);
  ...
  it("builds API model definition for M3 with 1M context and 64k max tokens", () => {
    ...
    expect(model.maxTokens).toBe(MINIMAX_M3_MAX_TOKENS);
    expect(model.input).toEqual(["text"]);
  });
});
