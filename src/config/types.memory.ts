...
export type MemoryConfig = {
  ...
  qmd?: MemoryQmdConfig;
  /** Enable structured fact extraction and injection into the memory prompt section. */
  factsEnabled?: boolean;
  /** Cron expression for facts extraction interval (6-field cron, e.g. "0 *\/30 * * * *" = every 30 min). */
  factsExtractionCron?: string;
  /** Max number of session files to process per extraction run per agent. */
  factsExtractionLimit?: number;
  /** How far back (in hours) to look for unprocessed sessions. */
  factsExtractionLookbackHours?: number;
};
